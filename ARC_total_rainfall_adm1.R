...
# rainfall by adm2, africa ####
   ...
   # first iteration takes a minute; subsequent ones take only a few seconds
...
save(total_rainfall_adm1, file = 'RainOerMe/total_rainfall_adm1.rda')

save( total_rainfall_adm1, file = paste0( source, '_adm1.rda') )
save(total_rainfall_adm1, file = paste0('RainOerMe/', source, '_adm1.rda') )



# Calculate change in rainfall from previous year, and baseline years, for each adm.  ####

load('total_rainfall_adm1.rda') # rainfall by month for each adm.

rain.baseline = # average for 2000-2006
   total_rainfall_adm1 %>% 
   filter( rain >= 0 ) %>%
   filter( year %in% 2000:2006 ) %>%
   group_by(country, adm1, month) %>%
...
tr1 = total_rainfall_adm1 %>% 
   ...
   arrange(country, adm1,month, year) %>%
   group_by(country, adm1, month) %>%
...
save(tr1, file = "tr1.rda")
save(tr1, file = "RainOerMe/tr1.rda")
